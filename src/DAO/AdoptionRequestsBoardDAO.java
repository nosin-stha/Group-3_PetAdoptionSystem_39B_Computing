...
public class AdoptionRequestsBoardDAO {
    ...
    // ── Get all pets + their requests grouped ────────────────────────────────
    ...
    // ── Search version ───────────────────────────────────────────────────────
    public Map<PetsData, List<AdoptionRequestData>> searchRequestsGrouped(int providerID, String keyword) {
        Map<PetsData, List<AdoptionRequestData>> map = new LinkedHashMap<>();
        Connection con = null;

        // Safe keyword escape — remove quotes to prevent injection
        ...
        String sql = "SELECT "
                ...
                + "LEFT JOIN AdoptionRequests ar ON p.petID = ar.petID "
                + "WHERE p.providerID = " + providerID + " "
                + "AND (p.petName LIKE '" + kw + "' "
                + "OR p.petType LIKE '" + kw + "' "
                + "OR ar.reqFullName LIKE '" + kw + "' "
                + "OR ar.adoptionStatus LIKE '" + kw + "') "
                + "ORDER BY p.petID, ar.adoptionID DESC";
        ...
        try {
            ...
            System.out.println("searchRequestsGrouped error: " + e);
        } finally {
            connector.closeConnection(con);
        }
    ...
    // ── Accept: request → Accepted, pet → Adopted ───────────────────────────
    ...
    // ── Decline: request → Declined, pet stays Available ────────────────────
