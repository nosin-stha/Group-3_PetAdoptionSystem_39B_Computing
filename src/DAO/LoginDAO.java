...
public class LoginDAO {
    ...
    public String getProviderBanReason(String username) {
        String reason = null;
        String sql =
            "SELECT ar.reportReason " +
            "FROM AccountReport ar " +
            "JOIN Providers p ON ar.providerID = p.providerID " +
            "WHERE p.proUsername = ? " +
            "ORDER BY ar.reportID DESC " +
            "LIMIT 1";
        ...
        try {
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                reason = rs.getString("reportReason");
            }
        } catch (Exception e) {
            System.out.println(e);
        } finally {
